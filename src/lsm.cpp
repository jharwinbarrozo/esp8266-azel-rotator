  ...
  switch (_type) {
    ...
    case LSM303DLHC:
      ...
      if (Wire.available() == 6) {
        ...
        byte  zh = Wire.read();
        //Assemble 16-bit values and perform the axis transformation
        gx = (int16_t)(yh << 8 | yl);       // gx = ((yh << 8) | yl);
        gy = -(int16_t)(xh << 8 | xl);      // gy = -((xh << 8) | xl);
        gz = -(int16_t)(zh << 8 | zl);      // gz = -((zh << 8) | zl);
        //Low pass filter the sensor data as it improves the calibration procedure
        gx = filGX.lpf(gx);
        gy = filGY.lpf(gy);
        gz = filGZ.lpf(gz);
  ...
  switch (_type) {
    ...
    case LSM303DLHC:
      ...
      if (Wire.available() == 6) {
        ...
        byte yl = Wire.read();
        //Assemble 16-bit values and perform the axis transformation
        mx = -(int16_t)(yh << 8 | yl);      // mx = -((yh << 8) | yl); -- this was the previous code
        my = (int16_t)(xh << 8 | xl);       // my = ((xh << 8) | xl); -- this was the previous code
        mz = (int16_t)(zh << 8 | zl);       // mz = ((zh << 8) | zl); -- this was the previous code
        //Low pass filter the sensor data as it improves the calibration procedure
        mx = filMX.lpf(mx);
        my = filMY.lpf(my);
        mz = filMZ.lpf(mz);
