...
void Mot::_fwdrev(float err) {
  analogWriteRange(255); // ESP8266 has 1024 resolution PWM, we need to change it to 255
  ...
  int spd = constrain(err * _gain, -255, 255);
  ...
  if (abs(err) < 3.5) {
    ...
    if (spd > 0) {
      analogWrite(_pin1, spd);
      analogWrite(_pin2, 0);
    } else {
      analogWrite(_pin1, 0);
      analogWrite(_pin2, -spd);
    }
  }
}
